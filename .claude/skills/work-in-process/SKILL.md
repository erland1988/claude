...
wip-check → 验证设计完整性
    ↓
wip-plan {module} → 生成执行计划
    ↓
wip-worktree create {module} → 创建独立工作区
    ↓
wip-code {module} → 编码（自动选择执行模式）
    ↓
wip-review {module} → 复核
    ↓
wip-worktree merge {module} → 合并到基分支
    ↓
...
| `wip-code` | 执行编码（子代理驱动） |
| `wip-review` | 编码后复核 |
| `wip-worktree` | Git Worktree 管理 |
| `wip-feishu` | 飞书文档管理 |

## 文件位置

- **子技能**: `skills/{subskill}/SKILL.md`
- **脚本**: `skills/{subskill}/scripts/` 或 `scripts/`
- **子代理**: `skills/wip-code/subagents/`
